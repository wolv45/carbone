...
carbone.renderCallback('image.docx', JSON.parse(JSON.stringify(dataDOCXImage)), null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultImage.docx', result);
});

carbone.render('image.docx', JSON.parse(JSON.stringify(dataDOCXImage)))
  .then(({ result }) => {
    fs.writeFileSync('resultImagePromisified.docx', result);
  })
...
carbone.renderCallback('html.docx', dataDOCXHTML, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultHTML.docx', result);
});
...
carbone.renderCallback('image.xlsx', dataXLSXImage, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultImage.xlsx', result);
});

const dataXLSXCharts = {
  table1: [
    {
      key: 'key1',
      value: 1
...
carbone.renderCallback('charts.xlsx', dataXLSXCharts, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultCharts.xlsx', result);
});

carbone.renderCallback('formulas.xlsx', dataXLSXCharts, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultFormulas.xlsx', result);
});


carbone.renderCallback('spaces.xlsx', dataXLSXCharts, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultSpaces.xlsx', result);
});

carbone.renderCallback('printArea.xlsx', dataXLSXCharts, null, function (err, result) {
  if (err) {
    return console.log(err);
  }
  // write the result
  fs.writeFileSync('resultPrintArea.xlsx', result);
});
...
const nestedData = {
  brands: [
    {
      brand: "Toyota",
      models: [{ size: "Prius 2", weight: 100 }, { size: "Prius 3", weight: 200 }]
    },
    {
      brand: "Tesla",
      models: [{ size: "Plade", weight: 300 }, { size: "Roadster", weight: 400 }]
    },
    {
      brand: "Renault",
      models: [{ size: "Sandero", weight: 500 }, { size: "Logan", weight: 600 }]
    },
    {
      brand: "Peugeot",
      models: [{ size: "406", weight: 600 }, { size: "608", weight: 700 }]
    }
  ]
}

carbone.render('nestedArrays.xlsx', nestedData)
  .then(({ result }) => {
    fs.writeFileSync('resultNestedArrays.xlsx', result);
  })
  .catch(err => console.error(err));